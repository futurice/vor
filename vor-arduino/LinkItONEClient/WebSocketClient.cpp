...
bool WebSocketClient::connect() {
    ...
    int c;
    bool sidfound = false;
    while ((c = socket_client->read()) != -1) {

        line += (char)c;

        if ((char)c == '\n') {
#ifdef DEBUGGING
            Serial.print("Got Header: " + line);
#endif
            if (!sidfound && line.startsWith("Set-Cookie: io=")) {
                sidfound = true;
                sid = line.substring(15,line.length() - 2); // Don't save last CR+LF
#ifdef DEBUGGING
                Serial.println("SID found: " + sid);
#endif
            }
            line = "";
        }
...
bool WebSocketClient::analyzeRequest() {
...
#endif
    socket_client->print(F("GET /socket.io/?EIO=3&transport=websocket&sid="));
    ...
    socket_client->print(CRLF);
    socket_client->print(F("Sec-WebSocket-Extensions: permessage-deflate; client_max_window_bits\r\n"));
    socket_client->print(CRLF);
    ...
    while ((bite = socket_client->read()) != -1) {
        ...
        if ((char)bite == '\n') {
            ...
            if (temp.startsWith("Sec-Websocket-Accept: ")) {
                ...
                Serial.println("Key found: " + serverKey);
            }
            temp = "";
        }
