...
int HttpClient::startRequest(const char* aServerName, uint16_t aServerPort, const char* aURLPath, const char* aHttpMethod, const char* aUserAgent, const char* contentType, const char* body)
{
    ...
    int ret = sendInitialHeaders(aServerName, IPAddress(0,0,0,0), aServerPort, aURLPath, aHttpMethod, aUserAgent, contentType, contentLength);
...
int HttpClient::startRequest(const IPAddress& aServerAddress, const char* aServerName, uint16_t aServerPort, const char* aURLPath, const char* aHttpMethod, const char* aUserAgent, const char* contentType, const char* body)
{
    tHttpState initialState = iState;
    if ((eIdle != iState) && (eRequestStarted != iState))
    {
    ...
    // Now we're connected, send the first part of the request
    int ret = sendInitialHeaders(aServerName, aServerAddress, aServerPort, aURLPath, aHttpMethod, aUserAgent, contentType, contentLength);
    if ((initialState == eIdle) && (HTTP_SUCCESS == ret))
    {
        if (body) {
            iClient->println();
...
int HttpClient::sendInitialHeaders(const char* aServerName, IPAddress aServerIP, uint16_t aPort, const char* aURLPath, const char* aHttpMethod, const char* aUserAgent, const char* contentType, uint64_t contentLength)
{
#ifdef LOGGING
    ...
    if (contentType) {
        sendHeader(HTTP_HEADER_CONTENT_TYPE, contentType);
    }
    if (contentLength > 0) {
        char buffer[8];
        sprintf(buffer, "%d", contentLength);
        sendHeader(HTTP_HEADER_CONTENT_LENGTH, buffer);
    }
...
void HttpClient::sendBasicAuth(const char* aUser, const char* aPassword)
{
    ...
    char input[3];
    char output[5]; // Leave space for a '\0' terminator so we can easily print
    ...
    for (int i = 0; i < (userLen+1+passwordLen); i++)
    {
        ...
        if ( (inputOffset == 3) || (i == userLen+passwordLen) )
        {
            ...
            int outputLen = base64_encode(output, input, inputOffset);
