...
WebSocketClient::WebSocketClient(const char *host, const char *path) : host(host), path(path) {
...
#endif
        if (analyzeSession() && analyzeRequest()) {
#ifdef DEBUGGING
...
bool WebSocketClient::analyzeSession() {
    String temp;
...
#ifdef DEBUGGING
    Serial.println(F("Sending session request"));
#endif
